...
// 2. Use the STABLE v1 endpoint (Gemma 3 was moved to stable last week)
function geminiUrl() {
  const key = process.env.GEMINI_API_KEY;
  // Use v1 instead of v1beta for Gemma 3
...
export async function runPredictBranch(body) {
  ...
  // ... [Your existing year logic stays exactly the same] ...
  let nextYear = baselineNode?.year ? Number(baselineNode.year) : (Number(lastMilestone.year) || Number(branchYear)) + (Math.random() < 0.5 ? 1 : 2);
  ...
  IMPORTANT: BE BRUTALLY REALISTIC. Show the actual cost of decisions. Most startups fail. Most indie games don't sell. Career changes mean pay cuts. High-paying jobs mean stress. The chances of you winning a lottery is extremely low. This is a LIFE SIMULATOR, not a wish fulfillment engine.
  ...
  const geminiResponse = await fetch(geminiUrl(), {
    ...
    body: JSON.stringify({
      contents: [
        { 
          role: "user",
          parts: [{text: "Output ONLY raw JSON. No markdown.\n\n" + systemPrompt}],
        },
        {
          role: "model",
          parts: [{text: "{"}]
  ...
  // 2. The Pre-fill Logic: 
  // If the AI didn't start with its own '{', add the one we pre-filled back in.
  // If the AI accidentally added its own '{' anyway, just use the text as-is.
  if (!jsonText.startsWith('{')) {
    jsonText = '{' + jsonText;
  }

  // 3. The "Markdown Guard":
  // In case the AI tried to wrap it in ```json blocks despite our instructions
  jsonText = jsonText.replace(/```json|```/g, "").trim();

  try {
    const prediction = JSON.parse(jsonText);
    
    // Return your successful response here...
    return {
      success: true,
      prediction,
      metadata: { generatedAt: new Date().toISOString(), model: MODEL_NAME, branchDecision }
    };
  } catch (parseError) {
    console.error("Malformed JSON received:", jsonText);
    throw new Error(`Failed to parse prediction: ${parseError.message}`);
  }
  ...
  return {
    ...
    metadata: {
      ...
      model: MODEL_NAME, // Return the string, not the object
      branchDecision
    }
  };
}

export { MODEL_NAME as GEMINI_MODEL };